...
<?php if ($resultHeadSetting['app_name'] == NULL || $resultHeadSetting['company'] == NULL || $resultHeadSetting['title'] == NULL) { ?>
	<title>Laporan Realisasi Mitra Tahun App Name | Company </title>
<?php } else { ?>
	<title>Laporan Realisasi Mitra Tahun <?= $_GET['TahunAju'] ?> - <?= $resultHeadSetting['app_name'] ?> | <?= $resultHeadSetting['company'] ?> -
				...
				<div class="row">
					<div class="col-sm-12" style="margin-top: 20px;margin-left:-10px">
						<div style="display: flex;justify-content: end;">
							<div>
								<form action="report_realisasi_per_tahun_pdf.php" target="_blank" method="GET">
							...
							<div class="btn-group m-r-5 m-b-5">
								...
								<div class="dropdown-menu dropdown-menu-right">
									<form action="report_realisasi_per_tahun_excel.php" target="_blank" method="GET">
										<input type="hidden" name="TahunAju" value="<?= $_GET['TahunAju'] ?>">
										<input type="hidden" name="me" value="<?= $_SESSION['username'] ?>">
				...
				<div class="panel-body text-inverse">
					...
					<div class="note note-default">
						...
						<div class="note-content">
							<?php
							$CountData = $dbcon->query("SELECT COUNT(*) AS total_report_all_mitra
														FROM tbl_cust_quota AS quo
														WHERE quo.quota_year='$WGetTahunAju'");
							$ResultCountData = mysqli_fetch_array($CountData);
							?>
							<h4 style="margin-top: 20px;">Total Data Mitra TBB Tahun <?= $WGetTahunAju ?>: <u><?= $ResultCountData['total_report_all_mitra'] ?> Mitra </u></h4>
							<p> Total Data Mitra TBB Tahun <?= $WGetTahunAju ?> berdasarkan data Nomor Pengajuan.</p>
						</div>
					</div>
					<div class="table-responsive">
						<?php
						$dataTable = $dbcon->query("SELECT quo.*,ref.NAMA AS NAMA_PENERIMA_BARANG,ref.NPWP
													FROM tbl_cust_quota AS quo 
													LEFT OUTER JOIN referensi_pengusaha AS ref ON ref.ID=quo.tbb_nama
													WHERE quo.quota_year='$WGetTahunAju'");
						...
						?>
								<table class="table table-striped table-bordetransparent table-td-valign-middle">
									<thead>
										<tr>
											...
											<th rowspan="3" style="text-align: center;"><?= $row['NAMA_PENERIMA_BARANG']; ?> <br> <?= $row['NPWP']; ?></th>
									...
									<tbody>
										<?php
										$keyIDPenerima = str_replace(['.', '-', ' '], ['', '', ''], $row['NPWP']);
										// Januari
										// 01
										// 01 CARTON GOL A
								...
								<div style="padding: 10px;background: transparent;margin-top: 10px;margin-bottom: 10px"></div>
							<?php } ?>
						<?php } else { ?>
							<table class="table table-striped table-bordetransparent table-td-valign-middle">
							</table>
