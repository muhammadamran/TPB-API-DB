...
if (isset($_POST["add_kuota"])) {
    ...
    if ($HasilCekValidasi != NULL) {
        echo "<script>window.location.href='adm_module.php?DataAlready=true';</script>";
    } else {
        ...
        $InputUsername        = $me;
        $InputModul           = 'Administrator Tools/Module';
        $InputDescription     = $me . " Insert Data: " .  $NameMitra . "-" .  $NameTahun . " ,Simpan Data Sebagai Log Module";
        $InputAction          = 'Insert';
        ...
        if ($query) {
            echo "<script>window.location.href='adm_module.php?InputSuccess=true';</script>";
        } else {
            echo "<script>window.location.href='adm_module.php?InputFailed=true';</script>";
        }
    }
}
// END CREATE Module
...
if (isset($_POST["NUpdateData"])) {
    ...
    $InputUsername        = $me;
    $InputModul           = 'Administrator Tools/Module';
    $InputDescription     = $me . " Update Data: " .  $UpdateMitra . "-" .  $UpdateTahun . ", Simpan Data Sebagai Log Module";
    $InputAction          = 'Update';
    $InputDate            = date('Y-m-d h:m:i');
    ...
    if ($query) {
        echo "<script>window.location.href='adm_module.php?UpdateSuccess=true';</script>";
    } else {
        echo "<script>window.location.href='adm_module.php?UpdateFailed=true';</script>";
    }
    // }
}
// END UPDATE Module
...
if (isset($_POST["NDeleteData"])) {
    ...
    $InputUsername        = $me;
    $InputModul           = 'Administrator Tools/Module';
    $InputDescription     = $me . " Hapus Data: " .  $DeleteMitra . "-" .  $DeleteTahun . ", Simpan Data Sebagai Log Module";
    $InputAction          = 'Hapus';
    $InputDate            = date('Y-m-d h:m:i');
    ...
    if ($query) {
        echo "<script>window.location.href='adm_module.php?DeleteSuccess=true';</script>";
    } else {
        echo "<script>window.location.href='adm_module.php?DeleteFailed=true';</script>";
    }
}
// END DELETE Module
?>
<?php if ($resultHeadSetting['app_name'] == NULL || $resultHeadSetting['company'] == NULL || $resultHeadSetting['title'] == NULL) { ?>
    <title>Module App Name | Company </title>
<?php } else { ?>
    <title>Module - <?= $resultHeadSetting['app_name'] ?> | <?= $resultHeadSetting['company'] ?> -
        ...
        <div class="col-xl-12">
            ...
            <div class="panel panel-inverse" data-sortable-id="ui-modal-notification-2">
                <div class="panel-heading">
                    <h4 class="panel-title"><i class="fas fa-info-circle"></i> [Administrator Tools] Module</h4>
                ...
                <div class="panel-body">
                    ...
                    <?php if ($resultForPrivileges['INSERT_DATA'] == 'Y') { ?>
                        <div class="css-button">
                            <?php include "modal/m_adm_module.php"; ?>
                        </div>
                    <?php } ?>
                    <!-- end css-button -->
                                                ...
                                                <div class="modal-content">
                                                    <form action="" method="POST">
                                                        <div class="modal-header">
                                                            <h4 class="modal-title">[Update Data] Module - <?= $row['NPWP'] ?>
                                                                <?= $row['NAMA'] ?></h4>
                                                                ...
                                                                <div class="row">
                                                                    <div class="col-md-8">
                                                                        <div class="form-group">
                                                                            <label for="IdMitraUpdate<?= $row['quota_id'] ?>">Nama
                                                                                Mitra </label>
                                                                            ...
                                                                            <select type="text" class="default-select2 form-control" name="UpdateMitra" id="IDMitraUpdate<?= $row['quota_id'] ?>">
                                                                                ...
                                                                                ?>
                                                                                    <option value="<?= $RowMitra['ID'] ?>">
                                                                                        <?= $RowMitra['NPWP'] ?> -
                                                                                        <?= $RowMitra['NAMA'] ?></option>
                                                                                <?php } ?>
                                                                            </select>
                                                                        </div>
                                                                    </div>
                                        ...
                                        <div class="modal fade" id="deleteData<?= $row['quota_id'] ?>">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <form action="" method="POST">
                                                        <div class="modal-header">
                                                            <h4 class="modal-title">[Hapus Data] Module - <?= $row['NPWP'] ?>
                                                                <?= $row['NAMA'] ?></h4>
                                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <div class="alert alert-danger m-b-0">
                                                                ...
                                                                <p>Anda tidak akan melihat data ini lagi, data akan di hapus
                                                                    secara permanen pada aplikasi!<br><i>"Silahkan
...
<script type="text/javascript">
    ...
    if (window?.location?.href?.indexOf('DataAlready') > -1) {
        ...
        history.replaceState({}, '', './adm_module.php');
    }
