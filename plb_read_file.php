...
<div style="color:transparent">
	...
	for ($row = 2; $row <= $rowcount; $row++) {
		...
		// START
		$descript 		 = explode(",", $subquery);
		$dataOKE		 = $descript[0];
		$descriptReplace = str_replace(['(', "'", '"', ' '], ['', '', '', ''], $dataOKE);
		$dataREF		 = $dbcon->query("SELECT COUNT(*) AS total FROM plb_header WHERE NOMOR_AJU LIKE '%$descriptReplace%'");
	...
	$cekRespon = $dbcon->query("SELECT ID,NOMOR_AJU FROM plb_header ORDER BY ID DESC LIMIT 1");
	...
	$query .= $dbcon->query("INSERT INTO plb_status
                       (ID,NOMOR_AJU_PLB,ck5_plb_submit)
                       VALUES
                       ('','$inputNoAJU','$inputck5_plb_submit')");
